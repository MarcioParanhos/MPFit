...
async function initPg() {
  ...
  // IMC records table: per-user BMI/weight/height history
  await pool.query(`
  ...
  `);
  console.log('initPg: ensured imc_records table');

  // Exercises table: catalog of known exercises with optional image and metadata
  await pool.query(`
  ...
  `);
  console.log('initPg: ensured exercises table');

  // Ensure workouts.exercise_id exists to reference exercises
  await pool.query(`ALTER TABLE workouts ADD COLUMN IF NOT EXISTS exercise_id INTEGER REFERENCES exercises(id) ON DELETE SET NULL;`);
  console.log('initPg: ensured workouts.exercise_id');
...
module.exports = {
  ...
  async getUserByEmail(email) {
    if (usePg) {
      const pool = await ensurePg(); const res = await pool.query('SELECT id, name, email, password_hash AS "passwordHash", created_at AS "createdAt", admin FROM users WHERE email=$1', [String(email).toLowerCase()]); return res.rows[0] || null;
    }
  ...
  async getUserById(id) {
    if (usePg) {
      const pool = await ensurePg(); const res = await pool.query('SELECT id, name, email, password_hash AS "passwordHash", created_at AS "createdAt", admin FROM users WHERE id=$1', [Number(id)]); return res.rows[0] || null;
    }
  ...
  async getWorkoutsByDay(dayId) {
    ...
    if (usePg) {
      const pool = await ensurePg();
      if (userId) {
        const res = await pool.query('SELECT id, day_id AS "day_id", exercise_id AS "exerciseId", name, planned_sets AS "plannedSets", planned_reps AS "plannedReps", youtube, current_weight AS "currentWeight", completed, position FROM workouts WHERE day_id=$1 AND user_id=$2 ORDER BY position NULLS LAST, id', [Number(dayId), Number(userId)]);
          return res.rows.map(r => ({ id: r.id, day_id: r.day_id, exerciseId: r.exerciseId || null, name: r.name, plannedSets: r.plannedSets, plannedReps: r.plannedReps, youtube: r.youtube, currentWeight: r.currentWeight === null ? null : Number(r.currentWeight), completed: r.completed, position: r.position }));
      }
      const res = await pool.query('SELECT id, day_id AS "day_id", exercise_id AS "exerciseId", name, planned_sets AS "plannedSets", planned_reps AS "plannedReps", youtube, current_weight AS "currentWeight", completed, position FROM workouts WHERE day_id=$1 ORDER BY position NULLS LAST, id', [Number(dayId)]);
        return res.rows.map(r => ({ id: r.id, day_id: r.day_id, exerciseId: r.exerciseId || null, name: r.name, plannedSets: r.plannedSets, plannedReps: r.plannedReps, youtube: r.youtube, currentWeight: r.currentWeight === null ? null : Number(r.currentWeight), completed: r.completed, position: r.position }));
    }
    const db = loadFile(); return db.workouts.filter(w => w.day_id === Number(dayId)).slice().sort((a,b)=> (a.position||0) - (b.position||0));
  },

  async addWorkout(dayId, { name, plannedSets, plannedReps, youtube, exerciseId }) {
    ...
    if (usePg) {
      ...
      // if an exerciseId is provided, prefer the canonical exercise name
      let finalName = name;
      if (exerciseId) {
        try {
          const ex = await pool.query('SELECT name FROM exercises WHERE id=$1', [Number(exerciseId)]);
          if (ex.rowCount > 0) finalName = ex.rows[0].name;
        } catch (e) {
          // ignore and fall back to provided name
        }
      }
      const res = await pool.query('INSERT INTO workouts(day_id, user_id, exercise_id, name, planned_sets, planned_reps, youtube, position) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id, day_id, exercise_id, user_id AS "userId", name, planned_sets AS "plannedSets", planned_reps AS "plannedReps", youtube, current_weight AS "currentWeight", completed, position', [Number(dayId), userId||null, exerciseId || null, finalName, Number(plannedSets||0), Number(plannedReps||0), youtube||null, nextPos]);
      const r = res.rows[0]; return { id: r.id, day_id: r.day_id, exerciseId: r.exercise_id || null, userId: r.userid, name: r.name, plannedSets: r.plannedsets, plannedReps: r.plannedreps, youtube: r.youtube, currentWeight: r.currentweight === null ? null : Number(r.currentweight), completed: r.completed, position: r.position };
    }
  ...
  async updateWorkout(workoutId, payload) {
    ...
    if (usePg) {
      const pool = await ensurePg();
      // Build a dynamic UPDATE that only sets fields provided in the payload.
      const sets = [];
      const values = [];
      let idx = 1;
      if (name !== undefined) { sets.push(`name=$${idx++}`); values.push(name); }
      if (plannedSets !== undefined) { sets.push(`planned_sets=$${idx++}`); values.push(Number(plannedSets||0)); }
      if (plannedReps !== undefined) { sets.push(`planned_reps=$${idx++}`); values.push(Number(plannedReps||0)); }
      if (youtube !== undefined) { sets.push(`youtube=$${idx++}`); values.push(youtube||null); }

      if (sets.length === 0) {
        // Nothing to update — return the current row
        const cur = await this.getWorkout(workoutId, userId);
        return cur;
      }

      // WHERE clause
      let where = ` WHERE id=$${idx++}`;
      values.push(Number(workoutId));
      if (userId) {
        where += ` AND user_id=$${idx++}`;
        values.push(Number(userId));
      }

      const sql = `UPDATE workouts SET ${sets.join(', ')}${where} RETURNING id, day_id AS "day_id", user_id AS "userId", name, planned_sets AS "plannedSets", planned_reps AS "plannedReps", youtube, current_weight AS "currentWeight", completed, position`;
  ...
  // IMC records helpers
  async addImcRecord(userId, { weight, height, bmi, date }) {
    if (usePg) {
      const pool = await ensurePg();
      const d = date || new Date().toISOString();
      const res = await pool.query('INSERT INTO imc_records(user_id, weight, height, bmi, date) VALUES($1,$2,$3,$4,$5) RETURNING id, user_id AS "userId", weight, height, bmi, date', [Number(userId), Number(weight), Number(height), Number(bmi), d]);
      return res.rows[0];
    }
    const db = loadFile(); db._ids.imc = (db._ids.imc||0) + 1; const r = { id: db._ids.imc, user_id: Number(userId), weight: Number(weight), height: Number(height), bmi: Number(bmi), date: date || new Date().toISOString() }; db.imc_records = db.imc_records || []; db.imc_records.push(r); saveFile(db); return r;
  },

  async getImcRecords(userId) {
    if (usePg) {
      const pool = await ensurePg();
      const res = await pool.query('SELECT id, user_id AS "userId", weight, height, bmi, date FROM imc_records WHERE user_id=$1 ORDER BY date DESC', [Number(userId)]);
      return res.rows.map(r => ({ id: r.id, userId: r.userid, weight: Number(r.weight), height: Number(r.height), bmi: Number(r.bmi), date: r.date }));
    }
    const db = loadFile(); db.imc_records = db.imc_records || []; return db.imc_records.filter(ir => ir.user_id === Number(userId)).slice().sort((a,b)=> new Date(b.date)-new Date(a.date));
  },

  async deleteImcRecord(id, userId) {
    if (usePg) {
      const pool = await ensurePg(); const res = await pool.query('DELETE FROM imc_records WHERE id=$1 AND user_id=$2', [Number(id), Number(userId)]); return res.rowCount>0;
    }
    const db = loadFile(); db.imc_records = db.imc_records || []; const idx = db.imc_records.findIndex(r => r.id === Number(id) && r.user_id === Number(userId)); if (idx === -1) return false; db.imc_records.splice(idx,1); saveFile(db); return true;
  },

  async clearImcRecords(userId) {
    if (usePg) {
      const pool = await ensurePg(); await pool.query('DELETE FROM imc_records WHERE user_id=$1', [Number(userId)]); return true;
    }
    const db = loadFile(); db.imc_records = (db.imc_records||[]).filter(r => r.user_id !== Number(userId)); saveFile(db); return true;
  },

  // Exercises helpers
  // signature: getExercises(search?)
  async getExercises() {
    const args = Array.from(arguments);
    const search = args.length ? args[0] : null;
    if (usePg) {
      const pool = await ensurePg();
      if (search && String(search).trim().length > 0) {
        // parameterized ILIKE search across name and target_muscle
        const q = `%${String(search).trim()}%`;
        const res = await pool.query('SELECT id, name, target_muscle AS "targetMuscle", equipment, image_path AS "imagePath", description, created_at AS "createdAt" FROM exercises WHERE name ILIKE $1 OR target_muscle ILIKE $1 ORDER BY name LIMIT 200', [q]);
        return res.rows.map(r => ({ id: r.id, name: r.name, targetMuscle: r.targetMuscle || null, equipment: r.equipment || null, imagePath: r.imagePath || null, description: r.description || null, createdAt: r.createdAt }));
      }
      const res = await pool.query('SELECT id, name, target_muscle AS "targetMuscle", equipment, image_path AS "imagePath", description, created_at AS "createdAt" FROM exercises ORDER BY name');
      return res.rows.map(r => ({ id: r.id, name: r.name, targetMuscle: r.targetMuscle || null, equipment: r.equipment || null, imagePath: r.imagePath || null, description: r.description || null, createdAt: r.createdAt }));
    }
    const db = loadFile(); db.exercises = db.exercises || [];
    if (search && String(search).trim().length > 0) {
      const q = String(search).toLowerCase().trim();
      return db.exercises.filter(ex => ((ex.name||'').toLowerCase().includes(q) || (ex.targetMuscle||'').toLowerCase().includes(q))).slice();
    }
    return db.exercises.slice();
  },

  async addExercise({ name, targetMuscle, equipment, imagePath, description }) {
    if (!name) throw new Error('name required');
    if (usePg) {
      const pool = await ensurePg();
      const res = await pool.query('INSERT INTO exercises(name, target_muscle, equipment, image_path, description) VALUES($1,$2,$3,$4,$5) ON CONFLICT (name) DO UPDATE SET target_muscle = COALESCE(excluded.target_muscle, exercises.target_muscle), equipment = COALESCE(excluded.equipment, exercises.equipment), image_path = COALESCE(excluded.image_path, exercises.image_path), description = COALESCE(excluded.description, exercises.description) RETURNING id, name, target_muscle AS "targetMuscle", equipment, image_path AS "imagePath", description, created_at AS "createdAt"', [String(name).trim(), targetMuscle || null, equipment || null, imagePath || null, description || null]);
      return res.rows[0];
    }
    const db = loadFile(); db._ids.exercises = (db._ids.exercises||0) + 1; const e = { id: db._ids.exercises, name, targetMuscle: targetMuscle||null, equipment: equipment||null, imagePath: imagePath||null, description: description||null, createdAt: new Date().toISOString() }; db.exercises = db.exercises || []; db.exercises.push(e); saveFile(db); return e;
  },

  async updateExercise(id, { name, targetMuscle, equipment, imagePath, description }) {
    if (!id) throw new Error('id required');
    if (usePg) {
      const pool = await ensurePg();
      const res = await pool.query('UPDATE exercises SET name = COALESCE($1,name), target_muscle = COALESCE($2,target_muscle), equipment = COALESCE($3,equipment), image_path = COALESCE($4,image_path), description = COALESCE($5,description) WHERE id=$6 RETURNING id, name, target_muscle AS "targetMuscle", equipment, image_path AS "imagePath", description, created_at AS "createdAt"', [name || null, targetMuscle || null, equipment || null, imagePath || null, description || null, Number(id)]);
      return res.rows[0] || null;
    }
    const db = loadFile(); db.exercises = db.exercises || []; const idx = db.exercises.findIndex(x=>x.id===Number(id)); if (idx === -1) return null; const ex = db.exercises[idx]; ex.name = name || ex.name; ex.targetMuscle = targetMuscle || ex.targetMuscle; ex.equipment = equipment || ex.equipment; ex.imagePath = imagePath || ex.imagePath; ex.description = description || ex.description; saveFile(db); return ex;
  },

  async deleteExercise(id) {
    if (!id) return false;
    if (usePg) {
      const pool = await ensurePg(); const res = await pool.query('DELETE FROM exercises WHERE id=$1', [Number(id)]); return res.rowCount>0;
    }
    const db = loadFile(); db.exercises = db.exercises || []; const idx = db.exercises.findIndex(x=>x.id===Number(id)); if (idx === -1) return false; db.exercises.splice(idx,1); saveFile(db); return true;
  },
  ...
  async cancelStart(dayId) {
    // signature: cancelStart(dayId, userId?) - clears startedAt/finishedAt/duration without marking completed
    const args = Array.from(arguments);
    const userId = args.length>1 ? args[1] : null;
    if (usePg) {
      const pool = await ensurePg();
      const res = userId ? await pool.query('UPDATE days SET started_at=NULL, finished_at=NULL, duration_seconds=NULL WHERE id=$1 AND user_id=$2 RETURNING id, user_id AS "userId", name, subtitle, completed, started_at AS "startedAt", finished_at AS "finishedAt", duration_seconds AS "durationSeconds"', [Number(dayId), Number(userId)]) : await pool.query('UPDATE days SET started_at=NULL, finished_at=NULL, duration_seconds=NULL WHERE id=$1 RETURNING id, name, subtitle, completed, started_at AS "startedAt", finished_at AS "finishedAt", duration_seconds AS "durationSeconds"', [Number(dayId)]);
      // also clear workout completion flags for this day (scoped to user if provided)
      if (userId) await pool.query('UPDATE workouts SET completed=false WHERE day_id=$1 AND user_id=$2', [Number(dayId), Number(userId)]);
      else await pool.query('UPDATE workouts SET completed=false WHERE day_id=$1', [Number(dayId)]);
      return res.rows[0] || null;
    }
    const db = loadFile(); const did = Number(dayId); const day = db.days.find(d => d.id === did); if (!day) return false; day.startedAt = null; day.finishedAt = null; day.durationSeconds = null; db.workouts.forEach(w => { if (w.day_id === did) w.completed = false; }); saveFile(db); return day;
  }
};

// expose initPg for one-off initialization scripts
module.exports.initPg = initPg;
