...
module.exports = {
  ...
  async getDays() {
    ...
    if (usePg) {
      const pool = await ensurePg();
      if (userId) {
        const res = await pool.query('SELECT id, name, subtitle, completed, started_at AS "startedAt", finished_at AS "finishedAt", duration_seconds AS "durationSeconds" FROM days WHERE user_id=$1 ORDER BY id', [Number(userId)]);
    ...
    const db = loadFile();
    if (userId) return db.days.filter(d => d.user_id === Number(userId));
  ...
  async addDay(name, subtitle) {
    ...
    if (usePg) {
      const pool = await ensurePg();
      const res = await pool.query('INSERT INTO days(user_id, name, subtitle) VALUES($1,$2,$3) RETURNING id, user_id AS "userId", name, subtitle, completed, started_at AS "startedAt", finished_at AS "finishedAt", duration_seconds AS "durationSeconds"', [userId||null, name, subtitle || null]);
      return res.rows[0];
    }
